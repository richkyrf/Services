 ...
 * @author JACK
 */
public class KaMekanik extends javax.swing.JFrame {
    ...
    String NamaMaster = "KaMekanik";
    String SelecQ = "SELECT `No` as 'No', `Nama` as 'Nama', if(`Status`=0,'Aktiv','Tidak Aktiv') as 'Status' FROM `tbkamekanik`";
    String SelecQ2 = "SELECT `No`, `Nama`, if(`Status`=0,'Aktiv','Tidak Aktiv') FROM `tbkamekanik`";
    String QInsert;
    String QUpdate;
    String QDelete;

    void GetQinsert() {
        QInsert = "INSERT INTO `tbkamekanik`(`Nama`, `Status`) VALUES ('" + JTPlat.getText() + "'," + (JCStatus.getSelectedIndex()) + ")";
    }

    void GetQupdate() {
        QUpdate = "UPDATE `tbkamekanik` SET `Nama`='" + JTPlat.getText() + "',`Status`= " + (JCStatus.getSelectedIndex()) + " WHERE `No`= " + jktable1.getint(jktable1.getSelectedRow(), 0) + "";
    }

    void getQDelete() {
        QDelete = "DELETE FROM `tbkamekanik` WHERE `No`=" + jktable1.getint(jktable1.getSelectedRow(), 0) + "";
    }
    ...
    {//GEN-HEADEREND:event_formWindowClosing
        F.VarJFrame.kaMekanik = null;        // TODO add your handling code here:
